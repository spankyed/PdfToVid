...
const extractPaperData = (record: any): Paper => {
    ...
    return {
        ...
        authors: metadata[0].authors[0].author.map((author: any) => `${author.keyname[0]} ${author.forenames ? author.forenames[0] : ''}`)
    };
};

const fetchAIPapersForDate = async (date: string): Promise<Paper[]> => {
    try {
        ...
        if (parsedData['OAI-PMH'] && parsedData['OAI-PMH'].error) {
            console.error('Error received from ArXiv OAI endpoint:', parsedData['OAI-PMH'].error[0]);
            return [];
        }
...
// Example usage
fetchAIPapersForDate('2023-09-21').then(papers => {
    console.log(papers);
}).catch(error => {
